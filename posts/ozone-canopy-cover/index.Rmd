...
description: "Assessing the relationship between ozone concentrations and percent tree canopy cover by census tract in LA County and the seasonality of ozone"
author:
  - name: Charlie Curtin
    affiliation: Master of Environmental Data Science Program
    affiliation-url: https://ucsb-meds.github.io/
date: 12-14-2023
image: la_treescape.png
format: 
      html: 
          code-fold: true
---
...
Github: https://github.com/charliecurtin1/ozone_tree_canopy_LA.git
...
tree_canopy <- read_csv(here("posts", "ozone-canopy-cover", "data", "2016 Tree Canopy Cover Calculations_a.csv"))

# read in CalEnviroScreen 4.0
enviroscreen <- st_read(here("posts", "ozone-canopy-cover", "data", "calenviroscreen40shpf2021shp", "CES4 Final Shapefile.shp"), quiet = TRUE) %>% 
...
# LA ozone 2017
la_ozone_2017 <- read_csv(here("posts", "ozone-canopy-cover", "data", "la_ozone_2017.csv")) %>% 
    rename(ozone = "Daily Max 8-hour Ozone Concentration")

# LA ozone 2018
la_ozone_2018 <- read_csv(here("posts", "ozone-canopy-cover", "data", "la_ozone_2018.csv")) %>% 
    rename(ozone = "Daily Max 8-hour Ozone Concentration")

# LA ozone 2019
la_ozone_2019 <- read_csv(here("posts", "ozone-canopy-cover", "data", "la_ozone_2019.csv")) %>% 
    rename(ozone = "Daily Max 8-hour Ozone Concentration")
...
1.  Fischetti, Mark. "The Paradox of Pollution-Producing Trees". ***Scientific American*** (June 2014): <https://www.scientificamerican.com/article/the-paradox-of-pollution-producing-trees/>

2.  Fitsky, Anne Charlott, Hans Sanden, Thomas Karl, et al. "The Interplay Between Ozone and Urban Vegetation-BVOC Emissions, Ozone Deposition, and Tree Ecophysiology". \***Frontiers in Forests and Global Change**, vol. 2 (2019): <https://doi.org/10.3389/ffgc.2019.00050>
